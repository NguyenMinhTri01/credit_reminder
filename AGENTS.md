...
- **PascalCase**: Classes, Interfaces, Enums, Types, Components
...
- **No trailing semicolons** (consistent with project)
...
- **Absolute imports** with @/ alias for local code
...
- **Validate at boundaries** - input validation on API entry
...
- **Mock external dependencies** - never hit real DB/API in tests
...
- **Use `as const`** for string constant objects to get literal types
...
- **Version synchronization** - update all packages together
