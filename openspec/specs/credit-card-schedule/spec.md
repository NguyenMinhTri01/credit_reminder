# Credit Card Schedule Specification
...
#### Scenario: Preview uses the configured application time zone across a calendar-day boundary
- **WHEN** the clock is `2026-09-06T20:00:00Z` (which is `2026-09-07` in
  `Asia/Ho_Chi_Minh`), the application time zone is `Asia/Ho_Chi_Minh`, `statementDay` is `5`, and
  `paymentDueDaysAfterStatement` is `1`
- **THEN** the preview displays `2026-10-06` regardless of the host operating-system time zone
